<?php
    ...
    function redirect2($path) {
        header("Location: http://95.111.98.17:6543/".$path);
    ...
    function update_user($db, $array) {
        ...
        if($array['sex_save'] == 'male') {
            $query = "UPDATE Boys SET salary = ".$array['salary'].", has_yaht = ".(($array['has_yaht'])? 'true' : 'false')
                .", has_villa = ".(($array['has_villa'])? 'true' : 'false').", car_model = '".$array['car_model']."'";
        }
        else {
        ...
        $query1 = "UPDATE User SET height = ".$array['height'].$passwd.", name = '".$array['name'].
            "', email = '".$array['email']."' WHERE user_id = ".$_SESSION['id'].";";
